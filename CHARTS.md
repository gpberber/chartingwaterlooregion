...
| `ggscalex_cont` | Continuous x scale | A numeric x axis; minor ticks only where the data have a figure. |
...
| `cwrlabel` | Direct text labels | cwr_label(), the house way to label anything inside the plot area: geom_text() with a halo behind the letters, so a label stays readable over a gridline, a line or a point. Text inside a filled bar stays geom_text(). |
...
Generated from `_dev/build_chart_references.R` on 2026-09-25. Do not edit this file by hand: add a snippet to `_dev/r.snippets`, describe it in that script's `catalogue` and `picker` tables, and run `Rscript _dev/build_chart_references.R`.
