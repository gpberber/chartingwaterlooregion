...
catalogue <- tribble(
  ...
  "building-blocks", "ggscalex_cont",      "Continuous x scale; minor ticks only where the data have a figure.",
  ...
  "building-blocks", "cwrlabel",           "cwr_label(): the house in-plot text label, haloed, with every positioning argument spelled out.",
...
picker <- tribble(
  ...
  "ggscalex_cont",       "block", "Continuous x scale",                "A numeric x axis; minor ticks only where the data have a figure.",
  ...
  "cwrlabel",            "block", "Direct text labels",                "cwr_label(), the house way to label anything inside the plot area: geom_text() with a halo behind the letters, so a label stays readable over a gridline, a line or a point. Text inside a filled bar stays geom_text().",
