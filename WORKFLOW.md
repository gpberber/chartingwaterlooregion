...
2.  Write `R/02_clean_data.R` to produce the tidy files the post needs in `data/`. Call `janitor::clean_names()` right after every read. Keep each output under 25 MB.
    ...
    **Long-form census data: an interval for every share.** The long form went to one household in four, so its counts are estimates, and the charts show shares, never counts (style rule 9a). Many census tables publish a 95% confidence interval for each count, as extra rows in a `Statistics` dimension. Keep those rows, and turn them into an approximate interval for every share the post charts with `R/census_ci.R` (the method and its limits are explained at the top of that file; `posts/commuting/R/02_clean_data.R` is the worked example). Each share then carries `percent_lower`, `percent_upper`, `cv` (its sampling error as a share of its value) and `unreliable` (a CV over a third). Claude then tells you, chart by chart, whether the intervals change what the chart says: where they do, the chart draws them; where they do not, a note states the widest. A share marked unreliable stays on the chart with a note key, because a small number can be realistic even when the sample cannot pin it down - you can still drop one. A table that publishes no intervals gets none, and a ranking drawn from it carries a note that close shares may differ only by sampling error. The README's Reliability table gets a **Sample estimates** row, which the post prints as its methodology note.
