...
chartingwaterlooregion/
  ...
  favicon.ico            browsers ask for this exact path whether a page names it or not
  _includes/             HTML pasted into every page: analytics.html and icons.html into the
                         <head>, share.html and like.html after the body
  ...
  _dev/                  tooling: safety check, git hook, snippet converter, r.snippets,
                         logo/make_logo.R (the whole visual identity), make_site_images.R
...
The slug becomes the URL (`.../posts/housing-starts/`), so keep it short, lowercase, hyphenated. Claude copies the template and creates:

```         
posts/housing-starts/
  index.qmd          the post, starts with draft: true
  README.md          key terms, data sources, reliability issues, licences, how to reproduce
...
Claude asks which kind of post it is, for a one-sentence description (shown on the listing card), and for two or three topic categories. Reuse existing categories where they fit; they are listed on the home page.
...
1.  Write `R/01_get_data.R` so it recreates `data-raw/` from scratch: download from the open-data portal, pull Statistics Canada tables with `cansim`, or fetch from a GitHub Release (section 6). Files you downloaded by hand from a portal still need a script that fetches them again.

2.  Write `R/02_clean_data.R` to produce the tidy files the post needs in `data/`. Call `janitor::clean_names()` right after every read. Keep each output under 25 MB.

    **Check the data-quality flags on what you keep.** Statistics Canada marks individual figures with symbols (E use with caution, F too unreliable to be published, x suppressed, A to D quality grades, p preliminary) and attaches footnotes, some of them about quality, to whole tables or to particular places and categories. A full table carries thousands of flags, nearly all on rows you will throw away, so the check runs after you have filtered to your places, years and categories and before you drop the flag columns. `R/data_quality.R` does it: `cwr_quality_flags()` prints every flag and quality footnote that applies and writes them to `data/quality_flags.csv`, one block per table, refreshed each time the script runs. The template shows the call; `01_get_data.R` saves each table's footnotes beside it with `get_cansim_table_notes()` so the check needs no internet. For a source other than Statistics Canada that has a flag column, pass `flag_cols =` and a `legend =` built from its documentation; one with caveats but no flag column just gets them written into the README's **Reliability** table.

    Claude reports every flag the check finds before building a chart from that data, and the decision is yours: a figure flagged F, x or `..` does not exist and is never charted as a number. **Figures flagged E ("use with caution") are never used at all**, and the check enforces it: `cwr_quality_flags()` hands back the data with every E, F, x and `..` figure blanked to missing, so the cleaning script has to carry on with what it returns (`kept <- kept |> cwr_quality_flags(...)`). Blanking rather than dropping rows is deliberate: a total built from a blanked part comes out missing, which is obvious, where a dropped row would quietly make it too small. A chart then shows a gap where the figure would be, and the Reliability table says why.

    **Every issue found goes in the README's `## Reliability` table**, which the post prints under the **Reliability** subheading of its "Data sources and reliability" section. It has two columns: the issue, in a sentence or two ending with the table number, and **Left out of the post because**. Closely related flags share one row - a table graded acceptable for one municipality, good for another and excellent for the rest is one issue, not three. That table is the disclosure: a figure Statistics Canada grades acceptable (D) is charted as it is, with no note on the chart saying so, because the post already prints the row explaining it. Claude drafts the rows; the wording is yours to change. To keep an issue out of the post, **do not delete its row**: write why in the second column, and it drops out of the post while the README keeps the issue and your reason. That way the repository always shows everything that was checked and what was set aside. If you delete a row anyway, Claude will remind you to put it back with a reason.

    **Each row ends with a hidden code** naming the flags it covers, for example `<!-- flags: 18-10-0004-01:note36 -->` (a symbol is `table:symbol`, a footnote `table:note` plus its number). Readers never see it. It is what lets the post check itself: **the post will not render while any flag in `data/quality_flags.csv` has no row carrying its code**, so a flag cannot reach the published post unconsidered. A row left out of the post with a reason still counts. A code for a flag the data no longer has gives a warning, so an out-of-date row gets noticed. The check runs whenever the post's code runs, which is why `/publish` renders each post going live on its own before the full site render.

    **Long-form census data: an interval for every share.** The long form went to one private household in four, and each published figure is Statistics Canada's weighted estimate for everyone in private households (people in collective dwellings such as nursing homes and student residences are not included). Charts show shares by default; a count only rounded and with its interval (style rule 9a). Many census tables publish a 95% confidence interval for each count, as extra rows in a `Statistics` dimension. Keep those rows, and turn them into an interval for every share the post charts with `R/census_ci.R`, which follows Statistics Canada's own method (explained at the top of that file; `posts/commuting/R/02_clean_data.R` is the worked example). Each share then carries `percent_lower`, `percent_upper`, `cv` (its sampling error as a share of its value) and `quality`: E for a CV of 16.6% to 33.3%, F above that. E and F shares are blanked, like Statistics Canada's own E and F figures, and the chart names the row "Not reported" with a note. Claude tells you, chart by chart, whether the intervals change what the chart says: where they do, the chart draws them; where they do not, a note states the widest. A table that publishes no intervals gets none, and a ranking drawn from it carries a note that close shares may differ only by sampling error. `01_get_data.R` also fetches each area's long-form total non-response rate (`cwr_census_tnr()`) and the topic's per-question non-response and imputation rates, and `02_clean_data.R` stops if an area's rate is 50% or more. None of this puts a sampling note on the charts: the README's Reliability table gets printed **Long-form census**, **Sample estimates** and **Response rates** rows, which are the post's methodology note.
...
- `data/tables.csv`: one row per table the post reads. Columns: `table` (a short name), `file` (path from the project root, so it can point into the post's `data/` or into `datasets/<name>/data/`), `description`, `source`, `licence`, `sample` (whether the figures are estimates from a sample: "Census long form, 25% sample of private households", a survey and its size, or "None (full count)"; it is printed beside the source in the bundle's README), `notes` (for example a required disclaimer).
- `data/dictionary.csv`: one row per column of every table. Columns: `table`, `column`, `description`, `units`. Types and example values are read from the data automatically. A column holding a confidence bound says so: "Lower bound of the 95% confidence interval for `workers`", in the same units as the estimate. The columns `R/census_ci.R` adds are described the same way: `percent_lower` and `percent_upper` as the approximate 95% interval for `percent` (percentage points), `cv` as its coefficient of variation (a proportion), `quality` as "E" (CV 16.6% to 33.3%) or "F" (above), with the share blanked.
...
The post's setup chunk (already in the template) sets `data_bundle_version <- 1`. Bump it whenever the data changes after publishing, so links in old copies of the post keep pointing at the data they were built from.

What readers get, at publish time (3.7), is `<slug>-data-v<n>.zip` attached to the post's GitHub Release, containing each table as CSV and Parquet, one Excel workbook when every table fits Excel, `data_dictionary.csv`, and a README with sources and licences. The post's Reproducibility box links to the zip; the dictionary itself is not printed in the post. It appears in two places, both generated from the data so neither can drift from it: the `## Data dictionary` section of the post's `README.md`, which is what somebody browsing the repository sees, and `data_dictionary.csv` in the zip, which is what somebody who downloads the data gets. `cwr_data_bundle()` rewrites the README section every time it builds; to refresh it on its own after changing a cleaning script, run `cwr_dictionary_readme("<slug>")`. Never type that table by hand. Bundles exist only for posts: a dataset in `datasets/` gets one only through the posts that use it, and only the tables those posts actually read.

### 3.4 Write the post

The template gives the structure: an opening paragraph with the question and the answer, sections with plain-language headings, one chart or table per section, a Key terms section (the official definition of each term used in a chart's title, subtitle or main body - never statistical or census geography terms, and never a term found only in a note or in the prose - printed from the README's `## Key terms` table: Claude drafts it from the sources' own documentation, leaves the Definition empty for any term it finds no official definition for, and you define or delete those; the render prints a WARNING line naming them until you do), a Data sources and reliability section (whose Sources and Reliability subheadings print the source table and the data-quality issues straight from the post's README.md), an optional Other sources section for anything cited that is not data, and a Reproducibility box at the end. Those last four are folded boxes (collapsed callouts) that the reader clicks to open, so the reference material stays out of the way; they do not appear in the "On this page" list.

- **Charts.** Ask Claude for the chart you want ("a ranked lollipop of housing starts per 1,000 residents for the Big 12, Waterloo Region bolded"). You do not have to name a template - describing what the chart should show is enough - but [CHARTS.md](CHARTS.md) lists every one of them with what it is good for, which is the quickest way to see what is possible. It is generated by `Rscript _dev/build_chart_references.R` from the same catalogue as the skill's templates, so it cannot drift; never edit it by hand. The same run installs the snippets into RStudio, so typing a template's name and pressing Shift+Tab gives the current version: it merges them into RStudio's own snippet file (in `%APPDATA%\RStudio\snippets\`), keeps any snippet only RStudio has, and leaves a dated backup of the file it replaced. Edit snippets in `_dev/r.snippets`, never in RStudio's copy, or the next run replaces the edit. The `cwr-charts` skill loads automatically: it picks a template from the snippet library, applies the house colours and theme, renders the chart to a PNG, looks at it, and tunes label positions before putting the code in the post. Every chart is built as `p` and handed to `cwr_figure(p, "fig-name", alt = )`, which draws it twice, once at the paragraph width for desktops and once at half that for phones, saves both PNGs in the post's `figures/` folder (committed), and writes the figure so the browser shows the right one. Text stays the same size in both, so phone readers never need to tap to enlarge. A chart with categories down its side - bars, lollipops - sets its own height from the number of rows, so bars are the same thickness in every chart on the site; only other charts, such as line charts and maps, are given a `height` and `phone_height`. The source line inside the chart comes from `cwr_caption("Source name")`, which names whoever published the numbers. It does not add your own name: a chart that plots a publisher's figures as published is their work. When the numbers on the chart were worked out here instead - a rate you calculated, an index you based, several sources combined - pass `credit = TRUE` and the line gains "\| Charting Waterloo Region".
...
- **Seeing a chart while you write it.** Run the chunk in RStudio and both versions of the chart appear, one after the other: the desktop chart and the phone chart, with the desktop one last. Checking the narrow version therefore costs nothing and needs no argument, which matters, because the phone version is what most readers see and the one that breaks. `draft_phone = TRUE` puts the phone version last instead.

  **Where they appear is an RStudio setting, not something the code controls.** With chunk output inline - the default, and the only option in Visual mode - both charts appear stacked under the chunk and the Plots pane stays empty. To get them in the Plots pane instead, where the back and forward arrows and the zoom button work on them, use Source mode and switch output to the console: the gear beside **Run** in the editor toolbar, **Chunk Output in Console**, or turn it off everywhere with Tools > Global Options > R Markdown > uncheck *Show output inline for all R Markdown documents*. They are the finished PNGs, not redraws, so labels sit exactly where they will on the site - the house style pins them to a fixed size, and a redraw at the pane's shape would put them somewhere else. Use the pane's zoom button for full size. Nothing is written to the post while drafting: the PNGs go to a temporary folder and the post's committed `figures/` are only touched by a real render.
...
Claude renders the post with drafts visible, fixes any R or Quarto errors, opens it in the browser pane, and screenshots it at desktop and phone widths. Or by hand:
...
Produces a findings list: prose numbers versus data, headings, chart style, captions and alt text, tables, `here()` paths, packages listed, README sources and licences, thumbnail, categories, reproducibility box, and a run of the safety check. Fix what it finds (Claude offers to), then re-run until clean.
...
Claude confirms the post should go public, flips `draft: false`, checks the data dictionary and builds and uploads the data bundle (3.3), renders the full site, runs the safety check, commits, pushes, deploys with `quarto publish gh-pages`, then opens the live URL and confirms it loads. The site updates within a minute or two. By hand:
...
Claude stages **everything in the folder**, runs the safety check, commits with a clear message, and pushes. "Everything" is literal: `/save` is `git add -A`, so any edit of your own that you have not committed goes into that commit too, under Claude's message. That is usually what you want at the end of a session. When it is not - you are midway through a rewrite, or your change deserves its own entry in the history - commit yours first and then ask for the save. Do this at the end of every working session even when nothing is ready to publish; drafts are invisible on the public site but safe on GitHub. By hand: `git add -A`, `git commit -m "message"`, `git push origin master`.
...
Write the two scripts as in 3.2, then list the objects in `load.R`, one line each with a comment saying what the table is. Prefer Parquet (`arrow::write_parquet()`) for any table over a few tens of MB; it is far smaller than `.rds` and posts can read only the columns they need with `arrow::open_dataset()`.
...
A dataset keeps its sources' data-quality flags and footnotes in `data/` so posts can check the rows they use (section 3.2). Tables with one row per figure keep the flag columns; tables pivoted wide by statistic list their flagged figures in a separate file instead, as `datasets/crime/R/03_quality_flags.R` does. A post that reads such a dataset puts the figures it uses back in long form, one row per figure, joins the flags file on by the dataset's keys (for crime: year, `geo_uid`, `ucr_code`, statistic), and runs `cwr_quality_flags(value_cols = "value")` once per source table; `posts/globe-csi-comparison/R/02_clean_data.R` is the worked example. Two things to get right there: footnotes name places and offences in Statistics Canada's own words, which the dataset has relabelled, so the post puts the published names back beside its figures for the footnotes to match; and any rule that trims the years or categories shown (globe-csi starts one offence in 2015) runs before the check, not after, or it reports flags on figures never charted. When Statistics Canada withdraws a table, its code, outputs and README rows come out everywhere; `/new-dataset` lists the places.

`datasets/crime/` is the worked example, with ten Statistics Canada tables, Ontario Financial Information Returns, and the WRPS occurrence data. The crime exploration notebooks (`explore.Rmd`, `test.qmd`, `test_snippets.qmd`) moved with it and live in `datasets/crime/R/exploration/`; open them from RStudio's Files pane.

### 5.4 Promote a post's pipeline to a dataset

If a second post wants data that lives inside an earlier post: move that post's `R/` scripts, `data-raw/`, and `data/` into `datasets/<slug>/`, rewrite the paths, write `load.R`, and point both posts at it. Claude does this on request; `/new-dataset` describes the steps.

## 6. Large data and GitHub Releases

Git is for code and small files. Anything over 25 MB, and any raw file that cannot be re-downloaded by script, goes to a GitHub Release: a tagged snapshot of the repository that carries attached files of up to 2 GB each. Readers download them without an account.
...
Claude uploads the files with `R/data_helpers.R`, adds the matching download call to the `01_get_data.R` script, adds big cleaned files to `.gitignore`, and updates the README. Tags follow `data-raw-<slug>-v1` and `data-<slug>-v1` (with `dataset-` inserted for datasets). When files change, use the next version so old renders stay reproducible.
...
The app lives in `posts/<slug>/app/app.R`, is deployed to shinyapps.io with `rsconnect`, and is embedded in the post with a responsive iframe. `/shiny-post <slug>` walks through building, testing locally, deploying, and embedding. The app reads only files inside `app/`.
...
- **The navbar has one worded link, About, and it sits on the right with the icons.** There is no `left:` key in `_quarto.yml` at all. There used to be a **Posts** item, and it pointed at `index.qmd` - which is exactly where the site name and mark already point, because the home page *is* the post listing. Two controls, one destination. If the home page ever becomes a welcome page with the listing on a separate page, that is when Posts earns its place back; until then, don't add it. Two things to re-check if the left side is ever repopulated: that the phone hamburger holds more than a single item, and that whatever page you are on still highlights its own link.

- **The strapline is written out in five places and they all have to agree.** "Learning about life in Waterloo Region, one chart at a time." lives in `subtitle` **and** `description-meta` in `index.qmd`, the hero block at the top of `about.qmd`, `description` in `_quarto.yml` (which feeds the RSS channel and the link-preview text of any page that sets none of its own), and the opening line of `README.md`. Change one, grep for the rest. **In `index.qmd` it must be `description-meta`, not `description`:** with `description` Quarto prints the sentence on the page underneath the subtitle, so the home page shows the tagline twice. **The social card no longer carries it** (removed 2026-09-14: a link preview shows the card image and the page description together, and the tagline appeared in both), so changing the wording no longer means re-running `make_logo.R` - which is also why the count went from six places to five. A separate, more descriptive line for search engines was tried the same day and dropped; Greg wants the one sentence everywhere and nowhere twice.
...
A post is reproducible when someone can clone the repo, run `source("R/packages.R"); install_missing()`, run the post's (or dataset's) `01_get_data.R` and `02_clean_data.R`, and `quarto render posts/<slug>` to get the same page. That holds when:
...
- no key is needed, or the README says which and how to get one (keys live in `~/.Renviron`, read with `Sys.getenv()`, never in code);
- the Reproducibility box at the end of the post prints package versions (`cwr_session_info()` does this without exposing local paths).
- `data/tables.csv` and `data/dictionary.csv` are complete, so the post offers a download bundle with a dictionary (`cwr_dictionary_check()` passes) and the post's README carries the generated `## Data dictionary` section.
...
| Refresh a post's README data dictionary | `Rscript -e 'source("R/data_bundle.R"); cwr_dictionary_readme("<slug>")'` |
| Rebuild chart templates and update RStudio's snippets after editing `_dev/r.snippets` | `Rscript _dev/build_chart_references.R` |
| Rebuild the logo, favicons and social card | `Rscript _dev/logo/make_logo.R` |
| See what is live versus what is committed | `git fetch origin` then compare `origin/master` with `origin/gh-pages` |
