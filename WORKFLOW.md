...
- **Charts.** Ask Claude for the chart you want ("a ranked lollipop of housing starts per 1,000 residents for the Big 12, Waterloo Region bolded"). The `cwr-charts` skill loads automatically: it picks a template from the snippet library, applies the house colours and theme, renders the chart to a PNG, looks at it, and tunes label positions before putting the code in the post. Every chart is built as `p` and handed to `cwr_figure(p, "fig-name", caption = , alt = , height = , phone_height = )`, which draws it twice, once at the paragraph width for desktops and once at half that for phones, saves both PNGs in the post's `figures/` folder (committed), and writes the figure so the browser shows the right one. Text stays the same size in both, so phone readers never need to tap to enlarge. The source line inside the chart comes from `cwr_caption("Source name")`.
...
- **Charts render in the wrong font.** Install Inter from https://rsms.me/inter/ once; `theme_cwr()` picks it up automatically and falls back to the default sans font otherwise.
