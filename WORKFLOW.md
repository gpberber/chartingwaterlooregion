...
- **There are two straplines now, on purpose (split 2026-09-14), and each has its own places.** The **visible tagline** - "Learning about life in Waterloo Region, one chart at a time." - is what readers see: `subtitle` in `index.qmd`, the hero block at the top of `about.qmd`, and two `card_text()` calls in `_dev/logo/make_logo.R`. Those three have to agree, and after changing them re-run `make_logo.R`, because the card is a picture and does not update itself. The **description** - "Charts and analysis about life in Waterloo Region, built from open data." - is what machines read, and it lives in three places: `description` in `_quarto.yml` (the RSS channel description and the fallback preview text for pages that set none), `description-meta` in `index.qmd` (the home page's `<meta name="description">`, which is what search engines read - use `description-meta`, not `description`, or Quarto prints it on the page under the subtitle), and the opening line of `README.md`, which is GitHub's shopfront. One surface stays on the tagline whatever you do: the home page's `og:description`, which Quarto takes from `subtitle`, so a pasted link shows the tagline. **Do not "tidy" the two into one.** A mood line makes a poor search result and a description makes a flat tagline; they were deliberately separated so each can do its own job.
